...
test.describe('options page', () => {
  ...
  test('persists a newly created template to chrome.storage', async ({ openExtensionPage, serviceWorker }) => {
    ...
    await expect
      .poll(async () => {
        // Templates now live one-per-key in local storage rather than inside the synced `template` namespace
        const stored = await serviceWorker.evaluate(() => chrome.storage.local.get());

        return JSON.stringify(stored).includes('E2E Template');
      })
      .toBe(true);
  });
  ...
  test('survives a reload of the page', async ({ openExtensionPage, serviceWorker }) => {
    ...
    await expect
      .poll(async () => {
        ...
        return JSON.stringify(stored).includes('Reload Survivor');
      })
      .toBe(true);
