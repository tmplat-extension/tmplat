...
export class SettingsService {
  ...
  async getSettings(): Promise<Settings> {
    const [analytics, appearance, logging, notification, oauth, template, urlShortener] = await allFulfilled([
  ...
  async saveSettings(settings: Readonly<Settings>): Promise<void> {
    const { analytics, appearance, general, logging, notification, oauth, urlShortener } = structuredClone(
      settings,
    ) as Settings;
