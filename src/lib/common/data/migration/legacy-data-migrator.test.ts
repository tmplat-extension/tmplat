  ...
  describe('migrate', () => {
    it('removes every legacy key it owns', async () => {
      ...
      expect(results).toEqual([
        { description: 'data_namespace_legacy_migration_step_1', outcome: DataMigrationStepOutcome.Passed },
      ]);
    ...
    it('does nothing when the step is not required', async () => {
      ...
      await expect(migrator.migrate(context)).resolves.toEqual([
        createNotRequiredStepResult('data_namespace_legacy_migration_step_1'),
      ]);
