...
describe('DataMigrationService', () => {
  ...
  describe('getRequiredMigrationVersions', () => {
    /*
     * Regression guard: A migration is named after the last version whose data format it consumes, so the '1.2.9'
     * migration applies to anyone arriving from 1.2.9 *or earlier* - their data is in the very format it reads. The
     * filter was inverted, so every one of the 33 releases before 1.2.9 was silently excluded and those users were
    ...
    /*
     * Regression guard: Completion is recorded against the version the *user* upgraded from, but the phase used to be
     * looked up by the migration version - which names a data format, so it never matched. A completed migration
     * therefore kept reporting itself as required, and `migrate.html` previewed an upgrade that had already happened.
     * The case above hides this by seeding and querying the same version.
     */
