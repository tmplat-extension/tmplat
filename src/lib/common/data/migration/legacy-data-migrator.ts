...
export class LegacyDataMigrator extends AbstractDataMigrator {
  ...
  protected getSteps(builder: DataMigrationStepBuilder): DataMigrationStep[] {
    return [
      builder.createSimpleStepForRemoval('1.2.9', 'migrate_namespace_legacy_migration_step_1', {
        // `analytics` is listed here, rather than transferred by a namespace of its own, because 2.0 dropped
        // analytics entirely. The key still has to be removed or it would be orphaned in the legacy storage
        // forever, since nothing else ever reads it again.
        keys: ['analytics', 'options_active_tab', 'options_limit', 'stats', 'updates'],
        storage: 'local',
      }),
    ];
  }
