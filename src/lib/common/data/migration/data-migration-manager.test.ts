...
const createMigrator = (
  ...
  ({
    ...
    migrate,
    namespace,
    namespaceTitle: `migrate_namespace_${namespace}`,
  }) as DataMigrator;
...
describe('DataMigrationManager', () => {
  ...
  /*
   * Regression guard: The context has to carry the *migration* version, not the version the user upgraded from: every
   * step opts in with `oldVersion === targetOldVersion` (see `DataMigrationStepBuilder`), so passing the user's version
   * made every step a silent no-op for anyone not arriving from exactly 1.2.9 - while still advancing the phase to
   * Completed, so it was never retried. It also disagreed with `getRequiredMigrations`, which the migrate UI uses to
   * preview those very same steps.
   */
        ...
        migrations: [
          {
            namespaces: [
              {
                namespace: DataNamespace.Appearance,
                namespaceTitle: 'migrate_namespace_appearance',
                steps: ['transfer tokens'],
              },
              {
                namespace: DataNamespace.Template,
                namespaceTitle: 'migrate_namespace_template',
