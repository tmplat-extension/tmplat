...
describe('DataMigrationStepBuilder', () => {
  ...
  describe('createSimpleStepForRemoval', () => {
    ...
    it('reports the legacy data when the removal fails', async () => {
      const step = builder.createSimpleStepForRemoval(OLD_VERSION, DESCRIPTION_KEY, {
        keys: ['a', 'b'],
        storage: 'local',
      });
      const {
        context,
        legacyData: { local },
      } = createContext({ local: { a: 1, c: 3 } });
      vi.spyOn(local, 'removeAll').mockRejectedValue(new Error('boom'));

      await expect(step.migrate(context)).resolves.toEqual({
        errors: [expect.objectContaining({ code: 'MIG500000' })],
        legacy: [{ data: 1, key: 'a', storage: 'local' }],
  ...
  describe('createSimpleStepForTransfer', () => {
    ...
    it('does not remove the legacy key when the mutation fails', async () => {
      ...
      // A thrown error is reported as a failed step carrying the legacy data, rather than rejecting, so the
      // migration UI can offer the unmigrated data to the user
      await expect(step.migrate(context)).resolves.toEqual({
        errors: [expect.objectContaining({ code: 'MIG500000' })],
        legacy: [{ data: [{ id: 'a' }], key: 'templates', storage: 'local' }],
        outcome: DataMigrationStepOutcome.Failed,
      });
      await expect(local.has('templates')).resolves.toBe(true);
    });

    it('reports a clone of the legacy data, so a mutator cannot taint what is reported', async () => {
      const { repository } = createRepositoryStub({ templates: [] });
      const step = builder.createSimpleStepForTransfer(
        OLD_VERSION,
        DESCRIPTION_KEY,
        ...
        repository,
        (_target, legacyData) => {
          (legacyData as { id: string }[])[0]!.id = 'tainted';
      ...
      await expect(step.migrate(context)).resolves.toMatchObject({
        legacy: [{ data: [{ id: 'a' }], key: 'templates', storage: 'local' }],
      });
    });
    ...
    describe('with a schema', () => {
      ...
      it('fails with MIG422000 and leaves the legacy key in place when the legacy data is invalid', async () => {
        const { data, repository } = createRepositoryStub({ templates: [] });
        const mutator = vi.fn<(data: TestData, legacyData: z.output<typeof schema>) => void>();
        const step = builder.createSimpleStepForTransfer(
          OLD_VERSION,
        ...
        await expect(step.migrate(context)).resolves.toMatchObject({
          ...
          legacy: [{ data: { enabled: 'yes' }, key: 'settings', storage: 'local' }],
          outcome: DataMigrationStepOutcome.Failed,
        });
    ...
    describe('when the mutator reports a non-passing outcome', () => {
      ...
      it('leaves legacy data reported by the mutator itself untouched', async () => {
        const detail: DataMigrationStepResultDetail = {
          errors: [ExtensionError.from('MIG500000')],
          legacy: [{ data: { partial: true }, key: 'templates', storage: 'local' }],
          outcome: DataMigrationStepOutcome.Failed,
        };
        const { step } = createCancellingStep(detail);
        const { context } = createContext({ local: { templates: [{ id: 'a' }] } });
