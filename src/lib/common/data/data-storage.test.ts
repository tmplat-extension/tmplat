...
/*
 * Chrome reports every quota failure as an untyped `Error` whose message names the limit, so the mapping onto an
 * `ExtensionError` code is message matching and nothing else. The distinction is what the user acts on: a per-item
 * breach means shortening the value, an area breach means deleting something, a rate breach means waiting.
 */
describe('quota failures', () => {
  const givenFailingArea = (error: unknown) => {
    const area = new FakeBrowserStorageArea();
    vi.spyOn(area, 'set').mockRejectedValue(error);
    vi.stubGlobal('browser', { ...getBrowserApiMock(), storage: { local: area } });

    return new BrowserDataStorage('local', asLoggingService());
  };

  it.each([
    ['QUOTA_BYTES_PER_ITEM quota exceeded', 'DAT507000'],
    ['QUOTA_BYTES quota exceeded', 'DAT507010'],
    ['MAX_ITEMS quota exceeded', 'DAT507010'],
    ['MAX_WRITE_OPERATIONS_PER_MINUTE quota exceeded', 'DAT429000'],
  ])('maps "%s" onto %s', async (message, code) => {
    const storage = givenFailingArea(new Error(message));

    await expect(storage.set('template:item:a', 'x')).rejects.toMatchObject({ code });
  });

  // The message itself is localized, so the substitution is asserted at the point it is supplied
  it('names the offending key so the user knows what to shorten', async () => {
    const intl = vi.spyOn(ExtensionError, 'intl');
    const storage = givenFailingArea(new Error('QUOTA_BYTES_PER_ITEM quota exceeded'));

    await expect(storage.set('template:item:a', 'x')).rejects.toThrow();

    expect(intl).toHaveBeenCalledWith('DAT507000', '"template:item:a"');
  });

  it('names every key of a failed batch write', async () => {
    const intl = vi.spyOn(ExtensionError, 'intl');
    const storage = givenFailingArea(new Error('QUOTA_BYTES_PER_ITEM quota exceeded'));

    await expect(storage.setAll({ a: 1, b: 2 })).rejects.toThrow();

    expect(intl).toHaveBeenCalledWith('DAT507000', '"a" "b"');
  });

  it('falls back to a generic write failure for an unrelated error', async () => {
    const storage = givenFailingArea(new Error('something else went wrong'));

    await expect(storage.set('a', 1)).rejects.toMatchObject({ code: 'DAT400000' });
  });

  // `DomDataStorage` has no quota strings to match on - the DOM signals the same condition with a named exception
  it('maps a DOM QuotaExceededError onto the area code', async () => {
    const area = new FakeDomStorage();
    vi.spyOn(area, 'setItem').mockImplementation(() => {
      throw new DOMException('exceeded', 'QuotaExceededError');
    });
    vi.stubGlobal('window', { localStorage: area });
    const storage = new DomDataStorage('localStorage', asLoggingService());

    await expect(storage.set('a', 1)).rejects.toMatchObject({ code: 'DAT507010' });
  });
});
