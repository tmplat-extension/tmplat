...
describe('OptionalDataRepository', () => {
  ...
  const givenStored = (data: unknown) => {
    storage = new FakeDataStorage(data === undefined ? {} : { [DataNamespace.Notification]: data });
  ...
  it('exposes its namespace', () => {
    expect(repository.namespace).toBe(DataNamespace.Notification);
  });
  ...
  describe('get', () => {
    ...
    it('reads only its own namespace', async () => {
      storage = new FakeDataStorage({ [DataNamespace.Notification]: VALID_DATA, other: 'ignored' });
      repository = new TestOptionalRepository(storage);
  ...
  describe('set', () => {
    it('persists valid data under its namespace', async () => {
      ...
      expect(storage.snapshot()).toEqual({ [DataNamespace.Notification]: VALID_DATA });
    });
    ...
    it('persists the schema output, so defaults and coercions are applied', async () => {
      ...
      expect(storage.snapshot()).toEqual({ [DataNamespace.Notification]: { count: 7 } });
    });
  });
  ...
  describe('init', () => {
    ...
    it('leaves existing data untouched and resolves false', async () => {
      ...
      expect(storage.snapshot()).toEqual({ [DataNamespace.Notification]: existing });
    });
  ...
  describe('mutate', () => {
    it('passes the current data to the mutator and stores the result', async () => {
      ...
      expect(storage.snapshot()).toEqual({ [DataNamespace.Notification]: { count: 2, name: 'a' } });
    });
    ...
    it('awaits an asynchronous mutator', async () => {
      ...
      expect(storage.snapshot()).toEqual({ [DataNamespace.Notification]: { count: 1, name: 'b' } });
    });
    ...
    describe('cancellation', () => {
      it('resolves true and persists when the mutator does not cancel', async () => {
        givenStored(VALID_DATA);

        await expect(repository.mutate((data) => ({ ...data, name: 'b' }))).resolves.toBe(true);

        expect(storage.snapshot()).toEqual({ [DataNamespace.Notification]: { count: 1, name: 'b' } });
      });

      it('resolves false and leaves the stored data untouched when the mutator cancels', async () => {
        givenStored(VALID_DATA);

        await expect(
          repository.mutate((data, cancel) => {
            cancel();
            return { ...data, name: 'b' };
          }),
        ).resolves.toBe(false);

        // A cancelled mutation must not partially apply, even though the mutator still returned data
        expect(storage.snapshot()).toEqual({ [DataNamespace.Notification]: VALID_DATA });
      });

      it('skips validation when cancelled, so a mutator can bail out having left the data invalid', async () => {
        givenStored(VALID_DATA);

        await expect(
          repository.mutate((_data, cancel) => {
            cancel();
            return { count: 1, name: '' };
          }),
        ).resolves.toBe(false);
      ...
      it('honours cancellation from an asynchronous mutator', async () => {
        givenStored(VALID_DATA);

        await expect(
          repository.mutate(async (data, cancel) => {
      ...
      it('ignores repeated cancellation', async () => {
        givenStored(VALID_DATA);
  ...
  describe('clear', () => {
    it('removes only its own namespace', async () => {
      storage = new FakeDataStorage({ [DataNamespace.Notification]: VALID_DATA, other: 'keep' });
      repository = new TestOptionalRepository(storage);
  ...
  describe('addChangeListener', () => {
    it('notifies the listener for changes to its own namespace', async () => {
      ...
      storage.notifyChanges({ [DataNamespace.Notification]: { newValue: VALID_DATA, oldValue: undefined } });
