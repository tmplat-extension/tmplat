...
export class AppearanceService {
  ...
  async getTemplateDataGridState(): Promise<AppearanceDataGrid> {
    const { templateDataGrid } = await this.repository.get();
    return templateDataGrid ?? DEFAULT_TEMPLATE_DATA_GRID_APPEARANCE;
  }

  async setTemplateDataGridState(templateDataGrid: AppearanceDataGrid): Promise<void> {
