...
describe('NotificationDataMigrator', () => {
  it('reports the notification namespace and title', () => {
    ...
    expect(migrator.namespaceTitle).toBe('migrate_namespace_notification');
  });

  describe('isMigrationRequired', () => {
    it('is required when upgrading from 1.2.9 with legacy notification data present', async () => {
      const { migrator } = createMigrator();
      const { context } = createMigrationContext({ legacyData: { local: { notifications: { enabled: false } } } });
    ...
    it('is not required from a different version', async () => {
      ...
      const { context } = createMigrationContext({
        legacyData: { local: { notifications: { enabled: false } } },
  ...
  it('returns the required migration step description when legacy notification data is present', async () => {
    const { migrator } = createMigrator();
    const { context } = createMigrationContext({ legacyData: { local: { notifications: {} } } });
  ...
  describe('migrate', () => {
    it('transfers the legacy notification flag and removes the legacy key', async () => {
      ...
      const {
        context,
        legacyData: { local },
      } = createMigrationContext({ legacyData: { local: { notifications: { enabled: false } } } });

      const results = await migrator.migrate(context);

      expect(results).toEqual([
        { description: 'migrate_namespace_notification_migration_step_1', outcome: DataMigrationStepOutcome.Passed },
      ]);
      expect(getTargetData(target)).toEqual({ ...SEED, enabled: false });
      await expect(local.has('notifications')).resolves.toBe(false);
    });

    it('fails with MIG422000 and retains the legacy key when the legacy notification data is garbage', async () => {
      const { migrator, target } = createMigrator();
      const {
        context,
        legacyData: { local },
      } = createMigrationContext({ legacyData: { local: { notifications: { enabled: 'no' } } } });
      ...
      await expect(local.has('notifications')).resolves.toBe(true);
    });
  });
});
