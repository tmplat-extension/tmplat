...
export class NotificationService {
  ...
  async notifyChangeLog(version: ExtensionVersion, versionDiffScope: VersionSegment | null): Promise<void> {
    ...
    await this.createNotification({
      ...
      message: this.intl.getMessage('changelog_notification_message', version),
      title: this.intl.getMessage('changelog_notification_title', this.intl.getMessage('app_name')),
    });
  }
  ...
  async notifyError(error: unknown, options: NotifyErrorOptions): Promise<string | undefined> {
    ...
    const notification: Notification = {
      ...
      title: options.title ?? this.intl.getMessage('app_error_title'),
    };
