...
describe('NotificationService', () => {
  ...
  it('creates a fixed-id change log notification when the version diff is within the configured scope', async () => {
    ...
    expect(intl.getMessage).toHaveBeenCalledWith('changelog_notification_message', '2.0.0');
    expect(intl.getMessage).toHaveBeenCalledWith('app_name');
    expect(intl.getMessage).toHaveBeenCalledWith('changelog_notification_title', 'app_name');
    expect(notifications.create).toHaveBeenCalledExactlyOnceWith('changelog', {
      iconUrl: 'chrome-extension://test-extension-id/img/icon_64.png',
      message: 'changelog_notification_message|2.0.0',
      title: 'changelog_notification_title|app_name',
      type: 'basic',
    });
  });
