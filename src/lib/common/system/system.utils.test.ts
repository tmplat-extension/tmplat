...
describe('system.utils with userAgentData (Microsoft Edge)', () => {
  // Edge reports three brands - the greased entry, the shared "Chromium" engine brand and its own product brand - so
  // it is the case that proves the brand picker returns the *product* rather than the engine. Edge installs the same
  // artifact as Chrome (from the Chrome Web Store), so this is the only place the two differ at runtime.
  const edgeNavigator = {
    platform: 'Windows',
    userAgentData: {
      platform: 'Windows',
      ...
      getHighEntropyValues: async () => ({
        fullVersionList: [
          ...
          { brand: 'Microsoft Edge', version: '153.0.3456.78' },
        ],
      }),
    },
  };

  it('picks the Edge product brand over the Chromium engine brand', () => {
    useNavigator(edgeNavigator);
    expect(system.getBrowserInfo()).toEqual({ name: 'Microsoft Edge', version: '153' });
  });

  it('resolves the Edge full version rather than the underlying Chromium version', async () => {
    useNavigator(edgeNavigator);
    await expect(system.getFullBrowserInfo()).resolves.toEqual({
      name: 'Microsoft Edge',
      version: '153.0.3456.78',
    });
  });
});
