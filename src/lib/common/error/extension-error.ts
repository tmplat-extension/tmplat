...
export class ExtensionError extends Error {
  ...
  private constructor(options: ExtensionErrorConstructorOptions) {
    super(options.messageOverride ?? ExtensionError.intl(options.code, ...(options.substitutions ?? [])), {
      cause: options.cause,
    });
  ...
  static intl(code: ExtensionErrorCode, ...substitutions: Substitutions): string {
    return localizeMessage(`xerr_${code.toLowerCase()}`, substitutions);
  }
}
