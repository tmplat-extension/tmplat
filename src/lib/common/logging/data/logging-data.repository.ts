  ...
  implements DataInstaller
{
  constructor(
    ...
    // Deliberately kept in sync storage. It is tiny and effectively fixed in size, and - unlike every other
    // namespace - it is read from content scripts, which cannot reach local storage because that area is restricted
    // to trusted contexts (see `ExtensionManager.restrictLocalStorageAccess`).
    super({
