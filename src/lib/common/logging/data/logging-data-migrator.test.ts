...
describe('LoggingDataMigrator', () => {
  it('reports the logging namespace and title', () => {
    ...
    expect(migrator.namespaceTitle).toBe('migrate_namespace_logging');
  });
  ...
  describe('migrate', () => {
    it('swaps legacy debug/info levels, deliberately ignores legacy enabled, and removes the legacy key', async () => {
      ...
      expect(results).toEqual([
        { description: 'migrate_namespace_logging_migration_step_1', outcome: DataMigrationStepOutcome.Passed },
      ]);
    ...
    it('fails with MIG422000 and retains the legacy key when the legacy logging data is garbage', async () => {
      ...
      expect(results).toEqual([
        {
          description: 'migrate_namespace_logging_migration_step_1',
          errors: [expect.objectContaining({ code: 'MIG422000' })],
          // Reported so the migration UI can show the user what could not be migrated
          legacy: [{ data: { enabled: 'yes', level: 999 }, key: 'logger', storage: 'local' }],
