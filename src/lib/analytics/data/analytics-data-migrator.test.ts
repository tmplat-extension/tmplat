...
const context = (legacy: Record<string, unknown>, oldVersion?: ExtensionVersion): DataMigrationContext =>
  createMigrationContext({ legacyData: { local: legacy }, oldVersion }).context;
  ...
  describe('migrate', () => {
    it('transfers the legacy boolean into the analytics enabled flag and removes the legacy key', async () => {
      ...
      expect(results).toEqual([
        { description: 'data_namespace_analytics_migration_step_1', outcome: DataMigrationStepOutcome.Passed },
      ]);
