...
import { type UrlShortenerBitlyProviderConfig } from 'extension/url-shortener/data/url-shortener-data.schema';
...
@injectable()
export class BitlyUrlShortenerProvider extends AbstractUrlShortenerProvider<UrlShortenerBitlyProviderConfig> {
  ...
  async shorten(url: URL): Promise<string> {
    ...
    const authentication = await this.oauthService.getAuthentication(OAuthProviderName.Bitly);
    if (!authentication) {
      // Should never happen as UrlShortenerService always calls isConfigured beforehand
