...
import { type UrlShortenerYourlsProviderConfig } from 'extension/url-shortener/data/url-shortener-data.schema';
...
const createConfig = (overrides: Partial<UrlShortenerYourlsProviderConfig> = {}): UrlShortenerYourlsProviderConfig => ({
...
describe('YourlsUrlShortenerProvider', () => {
  ...
  describe('isConfigured', () => {
    it('requires an installation URL', () => {
      expect(provider.isConfigured(createConfig({ url: null }))).toBe(false);
    });

    it('accepts a URL alone when no authentication mode is set', () => {
      expect(provider.isConfigured(createConfig())).toBe(true);
    });

    describe('advanced authentication', () => {
      const config = createConfig({ authenticationMode: YourlsAuthenticationMode.Advanced });

      it('requires a signature', () => {
        expect(provider.isConfigured({ ...config, signature: null })).toBe(false);
      });

      it('accepts a signature', () => {
        expect(provider.isConfigured({ ...config, signature: 'sig' })).toBe(true);
      });
    });

    describe('basic authentication', () => {
      const config = createConfig({ authenticationMode: YourlsAuthenticationMode.Basic });
      ...
      it.each([
        ...
        expect(provider.isConfigured({ ...config, password, username })).toBe(false);
      });

      it('accepts both a username and password', () => {
        expect(provider.isConfigured({ ...config, password: 'pass', username: 'user' })).toBe(true);
      });
    });
  });
  ...
  describe('shorten', () => {
    it('posts a form-encoded shorturl action to the configured installation', async () => {
      ...
      await provider.shorten(URL_TO_SHORTEN, createConfig());
    ...
    describe('advanced authentication', () => {
      ...
      it('sends a time-limited SHA-256 signature rather than the raw token', async () => {
        ...
        await provider.shorten(
          URL_TO_SHORTEN,
          createConfig({ authenticationMode: YourlsAuthenticationMode.Advanced, signature: 'raw-token' }),
        );
      ...
      it('produces a different signature as time passes, so it cannot be replayed indefinitely', async () => {
        ...
        await provider.shorten(URL_TO_SHORTEN, config);
    ...
    it('returns the shorturl from a successful response', async () => {
      ...
      await expect(provider.shorten(URL_TO_SHORTEN, createConfig())).resolves.toBe('https://yourls.example.com/abc');
    });
    ...
    it('does not constrain the shorturl host, since the installation is self-hosted', async () => {
      ...
      await expect(provider.shorten(URL_TO_SHORTEN, createConfig())).resolves.toBe('https://short.example.org/abc');
    });
    ...
    it('rejects a 200 response whose status is not success', async () => {
      ...
      await expect(provider.shorten(URL_TO_SHORTEN, createConfig())).rejects.toMatchObject({ code: 'SHO500000' });
    });

    it('rejects a successful response that is missing shorturl', async () => {
      fetchMock.mockResolvedValue(jsonResponse({ status: 'success' }));

      await expect(provider.shorten(URL_TO_SHORTEN, createConfig())).rejects.toMatchObject({ code: 'SHO422100' });
    });
    ...
    describe('when the URL has already been shortened', () => {
      it('treats the error:url response as a success and returns the existing shorturl', async () => {
        ...
        await expect(provider.shorten(URL_TO_SHORTEN, createConfig())).resolves.toBe(
    ...
    it('propagates a network failure', async () => {
      ...
      await expect(provider.shorten(URL_TO_SHORTEN, createConfig())).rejects.toThrow('Failed to fetch');
    });
  });
});
