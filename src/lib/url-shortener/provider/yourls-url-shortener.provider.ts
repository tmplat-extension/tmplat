...
@injectable()
export class YourlsUrlShortenerProvider extends AbstractUrlShortenerProvider<UrlShortenerYourlsProviderConfig> {
  ...
  isConfigured(config: Readonly<UrlShortenerYourlsProviderConfig>): boolean {
    if (!config.url) {
      return false;
    }
  ...
  async shorten(url: URL, config: Readonly<UrlShortenerYourlsProviderConfig>): Promise<string> {
    const urlString = url.toString();

    this.logger.trace(`Shortening URL via YOURLS:`, urlString);

    const response = await fetch(config.url!, {
      body: await this.createRequestBody(urlString, config),
      headers: {
  ...
  private async createRequestBody(url: string, data: Readonly<UrlShortenerYourlsProviderConfig>): Promise<string> {
