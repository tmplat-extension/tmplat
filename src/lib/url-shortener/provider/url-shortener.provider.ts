...
import { type UrlShortenerProviderConfig } from 'extension/url-shortener/data/url-shortener-data.schema';
...
export interface UrlShortenerProvider<Config extends UrlShortenerProviderConfig> {
  readonly name: UrlShortenerProviderName;

  isConfigured(config: Readonly<Config>): boolean | Promise<boolean>;

  shorten(url: URL, config: Readonly<Config>): Promise<string>;
}

export abstract class AbstractUrlShortenerProvider<
  Config extends UrlShortenerProviderConfig,
> implements UrlShortenerProvider<Config> {
  protected constructor(
    readonly name: UrlShortenerProviderName,
    protected readonly intl: IntlService,
  ) {}

  abstract isConfigured(config: Readonly<Config>): boolean | Promise<boolean>;

  abstract shorten(url: URL, config: Readonly<Config>): Promise<string>;
  ...
  protected createExtensionError(code: ExtensionErrorCode, cause?: unknown): ExtensionError {
    const providerIntlName = this.intl.getMessage(`shortener_name_${this.name}`);
