...
export class UrlShortenerService {
  ...
  constructor(
    ...
    private readonly providers: UrlShortenerProvider<UrlShortenerProviderConfig>[],
  ...
  async shorten(url: string | URL): Promise<string> {
    ...
    const providerIntlName = this.intl.getMessage(`shortener_name_${providerName}`);
    ...
    if (!(await provider.isConfigured(providerConfig))) {
      throw ExtensionError.from('SHO404100', providerIntlName);
    }

    try {
      return await provider.shorten(url, providerConfig);
    } catch (e) {
