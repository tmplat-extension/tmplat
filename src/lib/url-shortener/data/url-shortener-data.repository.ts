  ...
  implements DataInstaller
{
  constructor(
    ...
    // Any YOURLS credentials (signature, username, password) are stored alongside the provider selection, so this
    // deliberately uses local (device-only) storage rather than sync. This is not an inconsistency to be tidied away:
    // syncing it would push plaintext credentials through the user's Google account
    super({
