...
describe('UrlShortenerDataMigrator', () => {
  it('reports the URL shortener namespace and title', () => {
    ...
    expect(migrator.namespaceTitle).toBe('migrate_namespace_url_shortener');
  });
  ...
  describe('migrate', () => {
    it('discards legacy Bitly data without selecting Bitly, even when it was enabled', async () => {
      ...
      expect(results).toEqual([
        { description: 'migrate_namespace_url_shortener_migration_step_1', outcome: DataMigrationStepOutcome.Passed },
        createNotRequiredStepResult('migrate_namespace_url_shortener_migration_step_2'),
        createNotRequiredStepResult('migrate_namespace_url_shortener_migration_step_3'),
        createNotRequiredStepResult('migrate_namespace_url_shortener_migration_step_4'),
      ]);
    ...
    it('transfers legacy YOURLS settings, selects YOURLS when enabled, and removes the legacy key', async () => {
      ...
      expect(results).toEqual([
        createNotRequiredStepResult('migrate_namespace_url_shortener_migration_step_1'),
        { description: 'migrate_namespace_url_shortener_migration_step_2', outcome: DataMigrationStepOutcome.Passed },
        createNotRequiredStepResult('migrate_namespace_url_shortener_migration_step_3'),
        createNotRequiredStepResult('migrate_namespace_url_shortener_migration_step_4'),
      ]);
    ...
    it('migrates the untouched legacy YOURLS defaults, coercing blank strings to null', async () => {
      ...
      expect(results[1]).toEqual({
        description: 'migrate_namespace_url_shortener_migration_step_2',
    ...
    it('removes obsolete Google URL shortener data', async () => {
      ...
      expect(results).toEqual([
        createNotRequiredStepResult('migrate_namespace_url_shortener_migration_step_1'),
        createNotRequiredStepResult('migrate_namespace_url_shortener_migration_step_2'),
        { description: 'migrate_namespace_url_shortener_migration_step_3', outcome: DataMigrationStepOutcome.Passed },
        createNotRequiredStepResult('migrate_namespace_url_shortener_migration_step_4'),
      ]);
      expect(getTargetData(target)).toEqual(SEED);
    ...
    /*
     * Bitly was the only OAuth provider, so dropping it in 2.0.0 removed the OAuth mechanism altogether. Nothing
     * reads these keys any more, which is precisely why they have to be deleted here - `oauth2_bitly` holds an
     * access token, and without this step it would sit in local storage forever with no code path able to reach it.
     */
    it('removes the legacy OAuth credentials left behind by dropping Bitly', async () => {
      const { migrator, target } = createMigrator();
      const {
        context,
        legacyData: { local },
        ...
        legacyData: {
          local: {
            oauth2_adapterReverse: { bitly: 'bitly' },
            oauth2_bitly: { accessToken: 'secret-token', login: 'alice' },
            oauth2_google: { accessToken: 'another-secret' },
          },
        },
      });
      ...
      expect(results).toEqual([
        ...
        createNotRequiredStepResult('migrate_namespace_url_shortener_migration_step_2'),
        createNotRequiredStepResult('migrate_namespace_url_shortener_migration_step_3'),
        { description: 'migrate_namespace_url_shortener_migration_step_4', outcome: DataMigrationStepOutcome.Passed },
      ]);
      expect(getTargetData(target)).toEqual(SEED);
      await expect(local.has('oauth2_bitly')).resolves.toBe(false);
      await expect(local.has('oauth2_adapterReverse')).resolves.toBe(false);
      await expect(local.has('oauth2_google')).resolves.toBe(false);
    });
  });
});
