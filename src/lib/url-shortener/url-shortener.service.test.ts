...
import {
  ...
  type UrlShortenerProviderConfig,
...
type ProviderStub = {
  [K in keyof UrlShortenerProvider<UrlShortenerProviderConfig>]: UrlShortenerProvider<UrlShortenerProviderConfig>[K];
};

const createProviderStub = (
  name: UrlShortenerProviderName,
  overrides: Partial<ProviderStub> = {},
): UrlShortenerProvider<UrlShortenerProviderConfig> => ({
  name,
...
describe('UrlShortenerService', () => {
  ...
  let providers: UrlShortenerProvider<UrlShortenerProviderConfig>[];
  ...
  describe('shorten', () => {
    ...
    it('passes the selected provider its own configuration data', async () => {
      ...
      const [, configArg] = (providers[1].shorten as ReturnType<typeof vi.fn>).mock.calls[0];
    ...
    it('rejects with SHO404000 when the selected provider is not registered', async () => {
      ...
      repository.get.mockResolvedValue(createData(UrlShortenerProviderName.DaGd));
    ...
    it('localizes the provider name for the SHO404000 error', async () => {
      ...
      expect(intl.getMessage).toHaveBeenCalledWith('url_shortener_name_dagd');
    });

    it('rejects with SHO404100 when the provider reports its config is invalid', async () => {
      (providers[0].isConfigured as ReturnType<typeof vi.fn>).mockReturnValue(false);

      await expect(createService().shorten(LONG_URL)).rejects.toMatchObject({ code: 'SHO404100' });
      expect(providers[0].shorten).not.toHaveBeenCalled();
    });

    it('awaits an async isConfigured before deciding validity', async () => {
      (providers[0].isConfigured as ReturnType<typeof vi.fn>).mockResolvedValue(false);
