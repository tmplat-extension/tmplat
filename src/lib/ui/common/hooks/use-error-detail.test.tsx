...
function renderUseErrorDetail(fallbacks?: Parameters<typeof useErrorDetail>[0]) {
  ...
  const wrapper = ({ children }: { readonly children: ReactNode }) => (
    <IntlContext.Provider value={intl}>
      <ErrorDetailContext.Provider value={{ messageKey: 'app_error_unknown_message' }}>
        {children}
...
describe('useErrorDetail', () => {
  ...
  it('localizes the context fallback when no override or hook fallback is provided', () => {
    ...
    expect(result.current(undefined).message).toBe('localized:app_error_unknown_message');
    expect(intl.getMessage).toHaveBeenCalledWith('app_error_unknown_message');
  });
});
