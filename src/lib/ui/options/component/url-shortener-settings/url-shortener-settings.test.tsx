...
const setup = ({
  ...
  settings?: UrlShortenerSettingsValue;
} = {}) => {
  const onChange = vi.fn<(settings: UrlShortenerSettingsValue) => void>();
  const oauthService = {
    requestAuthentication: vi.fn(async () => bitlyAccount),
  };

  if (authenticationFails) {
    oauthService.requestAuthentication.mockRejectedValueOnce(new Error('No token for you'));
  }
  ...
  renderUi(<Harness />, { contexts: { oauthService } });

  return { oauthService, onChange, user: userEvent.setup() };
};
...
const setupDialog = ({
  ...
  renderUi(<SettingsDialog open onClose={vi.fn()} />, { contexts: { oauthService, settingsService } });
...
const openUrlShortenerPage = async (user: ReturnType<typeof userEvent.setup>) => {
  ...
  await screen.findByText('settings_url_shortener_bitly_title');
};
...
const chooseYourlsAuthenticationMode = async (user: ReturnType<typeof userEvent.setup>, optionName: string) => {
  await user.click(screen.getByRole('combobox', { name: 'settings_url_shortener_yourls_auth_field_label' }));
...
describe('UrlShortenerSettings', () => {
  describe('provider selection', () => {
    it('renders persisted provider and YOURLS fields', () => {
      ...
      expect(screen.getByRole('radio', { name: 'settings_url_shortener_yourls_radio_label' })).toBeChecked();
      expect(screen.getByRole('textbox', { name: 'settings_url_shortener_yourls_url_field_label' })).toHaveValue(
        'https://sho.rt/yourls-api.php',
      );
      expect(screen.getByLabelText('settings_url_shortener_yourls_signature_field_label')).toHaveValue(
        'secret-signature',
      );
      expect(screen.queryByLabelText('settings_url_shortener_yourls_username_field_label')).not.toBeInTheDocument();
    });

    it.each([
      ['Bitly', 'settings_url_shortener_bitly_radio_label', UrlShortenerProviderName.Bitly],
      ['spoo.me', 'settings_url_shortener_spoome_radio_label', UrlShortenerProviderName.SpooMe],
      ['da.gd', 'settings_url_shortener_dagd_radio_label', UrlShortenerProviderName.DaGd],
      ['YOURLS', 'settings_url_shortener_yourls_radio_label', UrlShortenerProviderName.Yourls],
  ...
  describe('Bitly OAuth account', () => {
    it('connects Bitly and writes the OAuth account in the settings OAuth schema shape', async () => {
      const { oauthService, onChange, user } = setup();

      await user.click(screen.getByRole('button', { name: 'settings_url_shortener_bitly_connect_button' }));

      expect(oauthService.requestAuthentication).toHaveBeenCalledExactlyOnceWith(OAuthProviderName.Bitly);
      expect(latestChange(onChange).oauth).toEqual({
        providers: { [OAuthProviderName.Bitly]: bitlyAccount },
      });
      expect(
        screen.getByRole('button', { name: 'settings_url_shortener_bitly_disconnect_button' }),
      ).toBeInTheDocument();
    });

    it('disconnects Bitly without revoking until settings are saved', async () => {
      const { onChange, user } = setup({
        settings: createUrlShortenerSettings({
          oauth: { providers: { [OAuthProviderName.Bitly]: bitlyAccount } },
        }),
      });

      await user.click(screen.getByRole('button', { name: 'settings_url_shortener_bitly_disconnect_button' }));

      expect(latestChange(onChange).oauth).toEqual({
        providers: { [OAuthProviderName.Bitly]: { accessToken: null, principal: null } },
      });
      expect(screen.getByRole('button', { name: 'settings_url_shortener_bitly_connect_button' })).toBeInTheDocument();
    });

    it('shows and dismisses an authentication error without writing a token', async () => {
      const { onChange, user } = setup({ authenticationFails: true });

      await user.click(screen.getByRole('button', { name: 'settings_url_shortener_bitly_connect_button' }));

      const alert = await screen.findByRole('alert');
      expect(alert).toHaveTextContent('No token for you');
  ...
  describe('YOURLS fields', () => {
    it('writes the URL field without changing sibling provider config', async () => {
      ...
      await typeInField(user, 'settings_url_shortener_yourls_url_field_label', 'https://new.example/yourls-api.php');
    ...
    it('writes the advanced signature field', async () => {
      ...
      await user.type(screen.getByLabelText('settings_url_shortener_yourls_signature_field_label'), 'abc123');
    ...
    it('writes basic username and password fields', async () => {
      ...
      await typeInField(user, 'settings_url_shortener_yourls_username_field_label', 'yourls-user');
      await user.type(screen.getByLabelText('settings_url_shortener_yourls_password_field_label'), 'yourls-pass');
    ...
    it('switches between no, basic and advanced authentication modes', async () => {
      ...
      expect(screen.getByLabelText('settings_url_shortener_yourls_username_field_label')).toBeInTheDocument();

      await chooseYourlsAuthenticationMode(user, 'settings_url_shortener_yourls_auth_advanced_option');
      ...
      expect(screen.getByLabelText('settings_url_shortener_yourls_signature_field_label')).toBeInTheDocument();
      expect(screen.queryByLabelText('settings_url_shortener_yourls_username_field_label')).not.toBeInTheDocument();

      await chooseYourlsAuthenticationMode(user, 'settings_url_shortener_yourls_auth_none_option');

      expect(
        latestChange(onChange).urlShortener.providers[UrlShortenerProviderName.Yourls].authenticationMode,
      ).toBeNull();
      expect(screen.queryByLabelText('settings_url_shortener_yourls_signature_field_label')).not.toBeInTheDocument();
    });
  });
  ...
  describe('validation', () => {
    it('shows required errors for the selected YOURLS provider', () => {
      ...
      expect(screen.getByText('settings_url_shortener_error_url_required')).toBeInTheDocument();
      expect(screen.getByText('settings_url_shortener_error_username_required')).toBeInTheDocument();
      expect(screen.getByText('settings_url_shortener_error_password_required')).toBeInTheDocument();
    });
    ...
    it('validates a configured YOURLS URL even when another provider is selected', () => {
      ...
      expect(getUrlShortenerSettingsErrors(settings.urlShortener, intl)).toEqual({
        url: 'settings_url_shortener_error_url_invalid',
      });
  ...
  describe('settings dialog integration', () => {
    ...
    it('saves the edited URL shortener and OAuth payload shape through SettingsService', async () => {
      const { settingsService, user } = setupDialog();
      await openUrlShortenerPage(user);

      await selectProvider(user, 'settings_url_shortener_yourls_radio_label');
      await typeInField(user, 'settings_url_shortener_yourls_url_field_label', 'https://save.example/yourls-api.php');
      await typeInField(user, 'settings_url_shortener_yourls_username_field_label', 'save-user');
      await user.type(screen.getByLabelText('settings_url_shortener_yourls_password_field_label'), 'save-pass');
      await user.click(screen.getByRole('button', { name: 'settings_url_shortener_bitly_connect_button' }));
      await screen.findByRole('button', { name: 'settings_url_shortener_bitly_disconnect_button' });
    ...
    it('reports load failures before the URL shortener page is available', async () => {
      ...
      expect(await screen.findByRole('alert')).toHaveTextContent('error_snackbar_unknown_message');
    ...
    it('reports save failures without swallowing the rejected save', async () => {
      ...
      await selectProvider(user, 'settings_url_shortener_spoome_radio_label');
      await user.click(screen.getByRole('button', { name: 'settings_dialog_apply_button' }));
