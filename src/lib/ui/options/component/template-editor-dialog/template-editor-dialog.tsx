...
export function TemplateEditorDialog({
  ...
  // saved.
  const dirty = !template || !isEqual(form, initialForm);
  const valid = !Object.keys(errors).length;
  const canSave = dirty && valid && !saving && !deleting;
  ...
  }, [form, getErrorMessage, onClose, onSaved, template, templateService]);
  ...
  }, [getErrorMessage, onClose, onSaved, template, templateService]);
      ...
      <Dialog open={open} onClose={onClose} maxWidth="md" fullWidth>
        ...
        <DialogActions sx={{ justifyContent: canDelete ? 'space-between' : 'flex-end' }}>
          ...
          <Stack direction="row" spacing={1}>
            ...
            <Button variant="contained" loading={saving} disabled={!canSave} onClick={handleSave}>
