...
describe('SettingsDialog', () => {
  ...
  it('shows a contextual error when settings fail to load', async () => {
    ...
    expect(await screen.findByText('app_error_unknown_message')).toBeInTheDocument();
  });
  ...
  // Regression: `useErrorDetail` used to return a new closure per render, and this dialog's loading effect lists
  ...
  it('keeps the selected page visible after navigating away from General', async () => {
    ...
    await userEvent.click(lastButton('settings_dialog_page_notifications'));

    expect(await screen.findByText('settings_notification_title')).toBeInTheDocument();
