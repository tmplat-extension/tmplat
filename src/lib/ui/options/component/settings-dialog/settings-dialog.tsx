...
export function SettingsDialog({ onClose, open }: SettingsDialogProps) {
  ...
  }, [getErrorDetail, open, settingsService]);
  ...
  const handleSave = useCallback(
    ...
    [getErrorDetail, onClose, settings, settingsService],
  );
  ...
  const dirty = !!settings && !isEqual(settings, savedSettings);
        ...
        [
          {
            ...
            page: (
              <UrlShortenerSettings
                settings={{ urlShortener: settings.urlShortener }}
                onChange={({ urlShortener }) => setSettings({ ...settings, urlShortener })}
              />
            ),
...
type SettingsPageId = 'appearance' | 'developer' | 'general' | 'notifications' | 'urlShorteners';
