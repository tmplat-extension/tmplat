...
export function TemplateDataGrid({ query = '' }: TemplateDataGridProps) {
  ...
  }, [getErrorDetail, refresh]);

  useEffect(() => {
    void loadTemplates();
    // Deliberately only run once on mount; retries are triggered explicitly via the error alert's retry action.
  ...
  useEffect(() => {
    ...
    appearance
      .getTemplateDataGridState()
      .then(({ columnVisibilityModel: hiddenColumns, pageSize }) => {
        if (!cancelled) {
      ...
      .catch((e: unknown) => {
        if (!cancelled) {
          // Non-critical: falls back to the defaults already used to initialise state above.
          console.warn('Failed to restore template data grid appearance state:', e);
        }
      });
  ...
  const handlePaginationModelChange = useCallback(
    (model: GridPaginationModel) => {
      ...
      appearance.setTemplateDataGridState({ columnVisibilityModel, pageSize: model.pageSize }).catch((e: unknown) => {
        setError(getErrorDetail(e).message);
      });
    },
    [appearance, columnVisibilityModel, getErrorDetail],
  );
  ...
  const handleColumnVisibilityModelChange = useCallback(
    (model: GridColumnVisibilityModel) => {
      ...
      appearance
        .setTemplateDataGridState({ columnVisibilityModel: model, pageSize: paginationModel.pageSize })
        .catch((e: unknown) => {
          setError(getErrorDetail(e).message);
        });
    },
    [appearance, getErrorDetail, paginationModel.pageSize],
  );
    ...
    <Box sx={{ m: '1em' }}>
      {error && (
        <Alert
          ...
          action={
            loadFailed ? (
              <Button color="inherit" size="small" onClick={() => void loadTemplates()}>
                {intl.getMessage('template_grid_retry_button')}
              </Button>
            ) : undefined
          }
        >
            ...
            <Stack direction="row" spacing={1} sx={{ alignItems: 'center', p: 1 }}>
              ...
              >
                ...
                >
                  ...
                  <ListItemText>{intl.getMessage('template_grid_bulk_move_top_option')}</ListItemText>
                </MenuItem>
                <MenuItem
                  onClick={() => {
                    setMoveMenuAnchor(undefined);
                ...
                >
                  ...
                  <ListItemText>{intl.getMessage('template_grid_bulk_move_bottom_option')}</ListItemText>
                </MenuItem>
              </Menu>
              <Button
        ...
        {rowMenu && (
          <>
            ...
            >
              ...
              <ListItemText>{intl.getMessage('template_grid_bulk_move_up_option')}</ListItemText>
            </MenuItem>
            <MenuItem
            ...
            >
              ...
              <ListItemText>{intl.getMessage('template_grid_bulk_move_down_option')}</ListItemText>
            </MenuItem>
            <MenuItem
              disabled={rowMenuAtBottom}
              onClick={() => {
