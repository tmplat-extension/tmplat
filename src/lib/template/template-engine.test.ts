...
const setup = ({
  ...
  const notificationService = {
    createNotification: vi.fn(async () => 'notification-id'),
...
describe('TemplateEngine', () => {
  ...
  it('rejects unsupported tab URLs, without rendering or copying', async () => {
    ...
    expect(notificationService.notifyError).toHaveBeenCalledExactlyOnceWith(
      expect.objectContaining({
        code: 'TEE403000',
      }),
      {
        messageKey: 'template_execution_fail_general_description',
        substitutions: ['Example template'],
  ...
  it('rejects invalid tab URLs asynchronously, so callers can catch the returned promise', async () => {
    ...
    expect(templateContextManagerFactory.createTemplateContextManager).not.toHaveBeenCalled();
    expect(clipboardService.copy).not.toHaveBeenCalled();
    expect(notificationService.notifyError).toHaveBeenCalledWith(expect.any(TypeError), {
      messageKey: 'template_execution_fail_general_description',
      substitutions: ['Example template'],
      title: 'template_execution_fail_title',
    });
  ...
  it('rejects render failures, notifies failure and leaves the clipboard unchanged', async () => {
    ...
    expect(clipboardService.copy).not.toHaveBeenCalled();
    expect(notificationService.notifyError).toHaveBeenCalledExactlyOnceWith(renderError, {
      messageKey: 'template_execution_fail_general_description',
      substitutions: ['Example template'],
      title: 'template_execution_fail_title',
    });
  ...
  // Regression guard. The failure notification used to be built and awaited here, so its own rejection replaced the
  // original error. Combined with the bug above this was worse than it looks: `createNotification` awaits both a
  // storage read and `browser.notifications.create`, so one broken dependency failed *both* notification calls and a
  // successful copy surfaced to the user as a notification/storage error. Reporting the failure is now delegated to
  // `NotificationService.notifyError`, which absorbs its own failures (see `notification.service.test.ts`).
  it('reports the failure without letting it replace the original template error', async () => {
    const { engine, notificationService } = setup({ context: {} });

    await expect(
      engine.execute(createOptions({ template: { ...TEMPLATE, content: '{missing}' } })),
    ).rejects.toMatchObject({ code: 'TEE400000' });

    expect(notificationService.notifyError).toHaveBeenCalledOnce();
  });

  it('suppresses the failure notification when requested', async () => {
    const { engine, notificationService } = setup({ context: {} });

    await expect(
      engine.execute(createOptions({ suppressNotifications: true, template: { ...TEMPLATE, content: '{missing}' } })),
    ).rejects.toMatchObject({ code: 'TEE400000' });
