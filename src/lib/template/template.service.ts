...
import {
  ...
  type TemplateDataTemplateBase,
  type TemplateDataTemplateUserDefined,
...
export class TemplateService {
  ...
  createTemplateActionInfo(data: TemplateData): TemplateActionInfo {
    if (data.action.mode === TemplateActionMode.Popup || !data.action.templateId) {
      // TODO: Should this throw an error instead when there's no data.action.templateId?
      return { mode: TemplateActionMode.Popup };
    }

    const template = TemplateService.toTemplates(data.templates).find((p) => p.id === data.action.templateId);
    if (!template) {
  ...
  createTemplateContextMenuInfo(data: TemplateData): TemplateContextMenuInfo {
    ...
    if (data.contextMenu.mode === TemplateContextMenuMode.Menu || !data.action.templateId) {
      // TODO: Should this throw an error instead when there's no data.action.templateId?
      return {
        ...contextMenuInfo,
  ...
  async removeTemplates(ids: readonly string[]): Promise<void> {
    ...
    await this.repository.mutate((data) => {
      ...
      if (data.action.templateId && removableIds.has(data.action.templateId)) {
        data.action.templateId =
          data.templates.find((template) => template.enabled)?.id ?? data.templates[0]?.id ?? null;
      }
  ...
  parseTemplates(value: string): TemplateTransferTemplate[] {
    ...
    const transfer = this.validationService.validateSchema(
      ...
      TemplateTransferSchema,
      {
        code: 'ERR500000', // TODO: Use specific error code
