  ...
  implements DataInstaller, DataUpdater
{
  ...
  install(_context: DataInstallContext): Promise<boolean> {
    return this.init(() => {
      ...
      return {
        ...
        templates: predefinedTemplates.map(toPredefinedTemplate),
      };
    });
  }
  ...
  private updatePredefinedTemplates(data: TemplateData): boolean {
    ...
    data.templates = data.templates
      ...
      .concat(
        predefinedTemplates.filter((template) => !existingPredefinedIds.has(template.id)).map(toPredefinedTemplate),
      );
