...
describe('TemplateDataMigrator', () => {
  ...
  describe('migrate', () => {
    ...
    /*
     * Ordering guards (MIGRATION-GAPS.md §2.1). 1.x stored an explicit `index` per template and sorted by it on
     * load (`background.coffee:2061`), so array position in the legacy data is NOT authoritative - these fixtures
     * deliberately give `index` an order that disagrees with the array order to prove which one is used.
     */
    describe('ordering', () => {
