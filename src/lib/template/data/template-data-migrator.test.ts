...
const createMigrator = (seed: TemplateData | Record<string, never> = SEED, realIdGenerator?: TemplateIdGenerator) => {
  const target = new FakeDataStorage({
    [DataNamespace.Template]: structuredClone(Object.keys(seed).length ? (seed as TemplateData) : SEED),
  });
  ...
  return { exclusionSnapshots, idGenerator: templateIdGenerator, logging, migrator, target };
};
...
const templateStepDescription = (step: number) => `data_namespace_template_migration_step_${step}`;
  ...
  describe('migrate', () => {
    ...
    it('fails the step and retains the legacy key when legacy link options are malformed', async () => {
      ...
      expect(results[0]).toMatchObject({
        description: 'data_namespace_template_migration_step_1',
    ...
    it('records the legacy key under migrations so a re-run is idempotent', async () => {
      ...
      const afterFirstRun = getTargetData(target).templates;
      ...
      const { migrator: second, target: secondTarget } = createMigrator(getTargetData(target));

      await second.migrate(context);

      expect(afterFirstRun).toHaveLength(3);
      expect(getTargetData(secondTarget).templates).toEqual(afterFirstRun);
    });
    ...
    it('drops a legacy shortcut that is already taken by an existing template', async () => {
      ...
      expect(getTargetData(target).templates[2]).toMatchObject({ id: TEMPLATE_ID, shortcut: null });
    });
    ...
    it('drops a legacy shortcut that is not a single upper-case alphanumeric character', async () => {
      ...
      expect(getTargetData(target).templates[2]).toMatchObject({ shortcut: null });
    });
    ...
    it.each([
      ...
      expect(getTargetData(target).templates[2]).toMatchObject({ shortcut: expected });
    });
    ...
    it('applies enabled and shortcut from a legacy predefined template to its bundled counterpart', async () => {
      ...
      expect(getTargetData(target).templates[1]).toEqual({
        enabled: false,
        id: 'PREDEFINED.00002',
        predefined: true,
        shortcut: 'Z',
    ...
    it('does not let a user-defined template reuse a shortcut just assigned to a predefined template', async () => {
      ...
      const templates = getTargetData(target).templates;

      expect(templates[1]).toMatchObject({ id: 'PREDEFINED.00002', shortcut: 'Z' });
      // 'Z' was claimed by PREDEFINED.00002 earlier in this very migration
      expect(templates[2]).toMatchObject({ id: TEMPLATE_ID, shortcut: null });
    });
    ...
    it('assigns a shortcut to a predefined template that had none', async () => {
      ...
      expect(getTargetData(target).templates[0]).toMatchObject({ id: 'PREDEFINED.00001', shortcut: 'Z' });
    });
    ...
    it('frees the shortcut a predefined template gave up, so a later template may claim it', async () => {
      ...
      expect(getTargetData(target).templates[2]).toMatchObject({ id: TEMPLATE_ID, shortcut: 'S' });
    });
    ...
    it('adds each generated ID to the exclusions so templates migrated together stay distinct', async () => {
      const { exclusionSnapshots, migrator, target } = createMigrator({}, new TemplateIdGenerator());
      const { context } = createMigrationContext({
        legacyData: {
          local: {
            templates: [
      ...
      const ids = getTargetData(target).templates.map((t) => t.id);
    ...
    it('fails the step and retains the legacy key when legacy templates are not an array', async () => {
      ...
      expect(results[4]).toMatchObject({
        description: 'data_namespace_template_migration_step_5',
        outcome: DataMigrationStepOutcome.Failed,
      });
    ...
    it('substitutes a placeholder for empty legacy content rather than failing every template', async () => {
      ...
      expectOnlyPassed(results, 5);
      expect(getTargetData(target).templates.map((t) => (t.predefined ? t.id : t.content))).toEqual([
        'PREDEFINED.00001',
        'PREDEFINED.00002',
        ' ',
        'Custom content',
    ...
    it('migrates only the first entry when a legacy key appears twice in the same array', async () => {
      ...
      const migrated = getTargetData(target).templates.filter((t) => !t.predefined);
    ...
    it('merges every step mutation when several legacy keys are present', async () => {
      ...
      expect(results).toEqual([
        { description: 'data_namespace_template_migration_step_1', outcome: DataMigrationStepOutcome.Passed },
        createNotRequiredStepResult('data_namespace_template_migration_step_2'),
        createNotRequiredStepResult('data_namespace_template_migration_step_3'),
        createNotRequiredStepResult('data_namespace_template_migration_step_4'),
        { description: 'data_namespace_template_migration_step_5', outcome: DataMigrationStepOutcome.Passed },
        { description: 'data_namespace_template_migration_step_6', outcome: DataMigrationStepOutcome.Passed },
      ]);
