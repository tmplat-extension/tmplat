...
export const TemplateSettingsSchema = z
  ...
  .meta({ id: 'TemplateSettings' });

/**
 * The template *settings*, which are small, bounded and worth syncing across devices.
 *
 * The templates themselves are deliberately **not** part of this schema: they are an unbounded collection of unbounded
 * content, so they are stored one key per template in `browser.storage.local` by `TemplateCollectionRepository`. See
 * MIGRATION-GAPS.md §1.7.
 */
