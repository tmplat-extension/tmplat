...
export const buildOptions = (data: TemplateContextData): TemplateContextOptions => {
  ...
  return {
    notifications: {
      changelog: {
        enabled: notificationData.changelog.enabled,
        scope: notificationData.changelog.scope,
      },
    ...
    templates: {
      ...
      contextMenu: {
        ...
        templateId: templateData.contextMenu.templateId,
      },
      links: {
...
export const createContentRenderer =
  ...
  () =>
  async (text, render) => {
    const content = await manager.render(text, render);

    return content || options.allowEmptyContent ? mapper(content, manager) : '';
  };

export type ContentRendererOptions = { allowEmptyContent?: boolean };
...
export const createTrimmedContentRenderer =
  ...
  () =>
  async (text, render) => {
    const content = await manager.renderTrim(text, render);
