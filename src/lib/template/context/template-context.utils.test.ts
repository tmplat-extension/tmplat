...
const renderContent = async (text?: string, renderer?: (template: string) => Promise<string>): Promise<string> =>
  text && renderer ? await renderer(text) : '';

const createManagerStub = (): TemplateContextManager =>
  ({
    render: renderContent,
    async renderTrim(text?: string, renderer?: (template: string) => Promise<string>) {
      return (await renderContent(text, renderer)).trim();
    },
...
const invokeRenderer = async (
  ...
  return text === undefined ? nested() : nested(text, render);
};
...
describe('createContentRenderer', () => {
  ...
  it('short-circuits without invoking the mapper when referenced bare', async () => {
    const mapper = vi.fn((content: string) => content);
    const definition = { render: createContentRenderer(mapper) };
  ...
  it('invokes the mapper with empty content when empty content is allowed', async () => {
    const mapper = vi.fn(() => 'fallback');
    const definition = { render: createContentRenderer(mapper, { allowEmptyContent: true }) };

    await expect(invokeRenderer(definition)).resolves.toBe('fallback');
...
describe('createTrimmedContentRenderer', () => {
  ...
  it('short-circuits without invoking the mapper when referenced bare', async () => {
    const mapper = vi.fn((content: string) => content);
    const definition = { render: createTrimmedContentRenderer(mapper) };
  ...
  it('invokes the mapper with empty content when empty content is allowed', async () => {
    const mapper = vi.fn(() => 'fallback');
    const definition = { render: createTrimmedContentRenderer(mapper, { allowEmptyContent: true }) };
