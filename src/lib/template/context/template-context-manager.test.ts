...
describe('TemplateContextManager', () => {
  ...
  describe('render helpers', () => {
    ...
    // A bare `{operation}` reaches its nested renderer with no arguments at all, which is what the engine now does
    // instead of stringifying the function - see MIGRATION-GAPS.md §8.
    it.each(['render', 'renderTrim', 'renderTrimStart', 'renderTrimEnd'] as const)(
      'resolves %s to an empty string when called with no arguments',
      async (method) => {
        const { manager } = createTestTemplateContextManager();

        await expect(manager[method]()).resolves.toBe('');
      },
    );
  });
  // `allFulfilled` defaults to reducing every rejection reason into an `AggregateError`, which is not an
  // `ExtensionError` and so would be flattened into a generic message by the time it reached the user - and would
  // not survive `MessageService`'s `toJSON` serialization across the message boundary at all. The fan-out here
  // therefore passes an explicit `firstError` reducer.
  describe('error reporting', () => {
    it('rejects getData with the underlying error rather than an AggregateError', async () => {
      const cause = ExtensionError.from('DAT404000', 'templates');
      const { manager } = createTestTemplateContextManager();
      vi.spyOn(manager['dataService'].local, 'getAny').mockRejectedValue(cause);

      await expect(manager.getData()).rejects.toBe(cause);
    });
  });
});
