...
export const lastModified: TemplateContextEntryDefinition = {
  ...
  categories: {
    [TemplateContextCategory.Operation]: {
      descriptionKey: 'entry_operation_last_modified',
      inputDataType: TemplateContextDataType.String,
      outputDataType: TemplateContextDataType.String,
      links: [LuxonFormattingTokensLink],
    },
    [TemplateContextCategory.Standard]: {
      descriptionKey: 'entry_standard_last_modified',
  ...
  render: createTrimmedContentRenderer(
    (content, manager) => {
      ...
      return manager.computeCacheIfAbsent(cacheKey, async () => {
        ...
        const { lastModified: tabLastModified } = await manager.getTabContext();
        const lastModifiedDateTime = (
          tabLastModified ? DateTime.fromJSDate(new Date(tabLastModified)) : manager.getCurrentDateTime()
        ).setLocale(locale);
