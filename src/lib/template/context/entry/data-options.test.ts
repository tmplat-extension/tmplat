...
describe('data and options context entries', () => {
  describe('options collection', () => {
    ...
    it('exposes the change log notification options', async () => {
      const { render } = createTestTemplateContextManager({
        data: { notification: { changelog: { enabled: true, scope: VersionSegment.Minor } } },
      });

      await expect(
        render('{#options}{#notifications}{#changelog}{enabled}|{scope}{/changelog}{/notifications}{/options}'),
      ).resolves.toBe('true|minor');
    });
  });
  ...
  describe('template counts', () => {
    ...
    it('resolves {templateCount} to the total number of templates', async () => {
      const { render } = createTestTemplateContextManager({ templates });
  ...
  describe('deprecated flat option entries', () => {
    ...
    /*
     * Bitly support was dropped in 2.0.0, but the entries are kept as stubs so a 1.x template referencing them still
     * renders instead of failing. They must stay inert whatever the URL shortener configuration says - the same
     * treatment `googl` gets for a service that no longer exists.
     */
    it('renders the deprecated Bitly entries as false now that Bitly is unsupported', async () => {
      const { render } = createTestTemplateContextManager({
        data: { url_shortener: { provider: UrlShortenerProviderName.Yourls } },
      });

      await expect(render('{bitly}|{bitlyAccount}')).resolves.toBe('false|false');
    });
