 ...
 * class behind the `cookies`/`hashSearchParams` bug and behind the bare-operation leak, both of which are fixed in this
 * codebase.
 */
...
describe('template context entry contract', () => {
  ...
  describe('operation entries', () => {
    ...
    /**
     * Regression guard for the bare-operation leak.
     *
    ...
    /**
     * Regression guard: These six were declared as `Collection` entries with an `Array` data type, so the guide
     * promised iteration (`{#selectAll}{.}{/selectAll}`) that the engine can never deliver: they are section lambdas,
     * so the body is consumed as the expression and any array they return is stringified rather than iterated.
     * Returning an array therefore only chose the separator, and chose a *different* one from 1.x — which joined with a
     * new line.
     */
