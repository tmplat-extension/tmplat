...
describe('tab metadata context entries', () => {
  ...
  describe('link target', () => {
    ...
    it('converts the link target html to Markdown for {linkMarkdown}', async () => {
      const { convert, render } = withTabContext({ linkTarget: { html: '<a href="/x">x</a>', text: 'x' } });

      await expect(render('{linkMarkdown}')).resolves.toBe('markdown:<a href="/x">x</a>');
      expect(convert).toHaveBeenCalledWith('<a href="/x">x</a>', { inline: false });
    });
  });
