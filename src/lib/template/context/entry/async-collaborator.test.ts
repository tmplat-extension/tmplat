...
describe('async collaborator context entries', () => {
  ...
  describe('URL shortening', () => {
    ...
    it('shortens the current page URL when {shorten} is referenced bare', async () => {
      const { render, shorten } = createTestTemplateContextManager({
        shortUrl: 'https://sho.rt/3',
        url: 'https://example.com/page',
      });

      await expect(render('{shorten}')).resolves.toBe('https://sho.rt/3');
      expect(shorten.mock.calls[0][0].toString()).toBe('https://example.com/page');
    });
  });
  ...
  describe('page querying', () => {
    ...
    // The new line separator matches tmplat 1.x (`src/lib/background.coffee:1238`), so a migrated 1.x template
    // keeps producing the same output. See MIGRATION-GAPS.md §8.
    it('resolves {#selectAll} to a new-line-joined string of every result', async () => {
      const { render } = createTestTemplateContextManager({ tabContent: ['a', 'b', 'c'] });

      await expect(render('{#selectAll}li{/selectAll}')).resolves.toBe('a\nb\nc');
    });
  ...
  describe('markdown conversion', () => {
    ...
    // tmplat 1.x joined the matched HTML and converted the whole thing once (`src/lib/background.coffee:1237-1239`).
    // Converting each fragment separately restarts the converter's document-level state, so its reference-style
    // link definitions would be emitted once per match rather than once per template. See MIGRATION-GAPS.md §8.
    it('converts every {#selectAllMarkdown} result as a single document', async () => {
      const { convert, render } = createTestTemplateContextManager({ tabContent: ['<b>a</b>', '<i>b</i>'] });

      await expect(render('{#selectAllMarkdown}li{/selectAllMarkdown}')).resolves.toBe('markdown:<b>a</b>\n<i>b</i>');
