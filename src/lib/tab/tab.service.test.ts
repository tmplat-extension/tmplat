...
const CONTENT_INPUT_ALL = { ...CONTENT_INPUT, queryAll: true } as const;
...
describe('TabService', () => {
  ...
  const createService = (
    configs: Array<MessageConfig<unknown, unknown>> = [TabContentMessageConfig, TabContextMessageConfig, VOID_CONFIG],
  ) => {
  ...
  describe('getTabContent', () => {
    it('sends a TabContent message and returns the validated output', async () => {
      ...
      getBrowserApiMock().tabs.sendMessage.mockResolvedValue({
        data: { output: 'a', queryAll: false },
        id: RESPONSE_ID,
        result: 'success',
      });

      await expect(service.getTabContent(3, CONTENT_INPUT)).resolves.toBe('a');
    ...
    it('returns every match when queryAll is set', async () => {
      const service = createService();
      getBrowserApiMock().tabs.sendMessage.mockResolvedValue({
        data: { output: ['a', 'b'], queryAll: true },
        id: RESPONSE_ID,
        result: 'success',
      });

      await expect(service.getTabContent(3, CONTENT_INPUT_ALL)).resolves.toEqual(['a', 'b']);
    });

    // The output schema is discriminated on `queryAll`, so a content script answering a single-match query with an
    // array (or the reverse) is reported rather than silently coerced by the caller. Consumers narrowed the union
    // by hand before this, which meant a regression in the content script was invisible.
    it.each([
      ['an array for a single-match query', CONTENT_INPUT, { output: ['a', 'b'], queryAll: false }],
      ['a string for an all-match query', CONTENT_INPUT_ALL, { output: 'a', queryAll: true }],
      ['an output that contradicts the request', CONTENT_INPUT, { output: ['a'], queryAll: true }],
    ])('rejects %s with MSG422000', async (_label, input, data) => {
      const service = createService();
      getBrowserApiMock().tabs.sendMessage.mockResolvedValue({ data, id: RESPONSE_ID, result: 'success' });

      await expect(service.getTabContent(3, input)).rejects.toThrow(
        expect.objectContaining({ code: 'MSG422000' }) as Error,
      );
    });
  });
