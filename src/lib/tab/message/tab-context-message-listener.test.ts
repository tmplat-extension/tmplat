...
const createContainer = () => {
  ...
  return {
    ...
    // `absolutizeUrls` asks the container itself whether it carries a url attribute before descending into it.
    hasAttribute: () => false,
    get innerHTML() {
    ...
    querySelectorAll: (selector: string) => {
      switch (selector) {
        // The url-rewriting pass; covered against real elements in `tab-context-message-listener.urls.test.ts`,
        // since resolving a url against the document's base is a claim only a real DOM can answer.
        case '[href],[src]':
          return [];
        case 'img[src]':
          return appended.images ?? [];
        default:
          return appended.links ?? [];
      }
    },
  };
};
...
describe('TabContextMessageListener', () => {
  ...
  describe('linkTarget', () => {
    ...
    // The anchor is cloned before its urls are rewritten, so the live node in the user's page is never touched. Both
    // halves are asserted against real elements in `tab-context-message-listener.urls.test.ts`.
    it('captures the enclosing anchor html and text', async () => {
      installDom();
      const clone = { hasAttribute: () => false, outerHTML: '<a href="/x">link</a>', querySelectorAll: () => [] };
      const anchor = { cloneNode: () => clone, outerHTML: '<a href="/x">link</a>', textContent: 'link' };
      const targetHolder = new ContextMenuTargetHolder();
