...
describe('TabContentMessageListener', () => {
  describe('selector expressions', () => {
    it('returns the text content of the first match', async () => {
      ...
      await expect(createListener().onMessage(selectorInput())).resolves.toEqual({ output: 'Hi', queryAll: false });
    });
    ...
    it('returns the inner HTML of the first match when the html format is requested', async () => {
      ...
      ).resolves.toEqual({ output: '<b>Hi</b>', queryAll: false });
    });

    it('returns an empty string when nothing matches', async () => {
      stubDocument({ querySelector: vi.fn(() => null) });

      await expect(createListener().onMessage(selectorInput())).resolves.toEqual({ output: '', queryAll: false });
    });
  ...
  describe('xpath expressions', () => {
    ...
    it('stringifies a boolean result', async () => {
      ...
      await expect(createListener().onMessage(xpathInput())).resolves.toEqual({ output: 'true', queryAll: false });
    });

    it('stringifies a number result', async () => {
      givenXPathResult({ resultType: XPATH_RESULT.NUMBER_TYPE, numberValue: 42 });

      await expect(createListener().onMessage(xpathInput())).resolves.toEqual({ output: '42', queryAll: false });
    });

    it('returns a string result verbatim', async () => {
      givenXPathResult({ resultType: XPATH_RESULT.STRING_TYPE, stringValue: 'hello' });

      await expect(createListener().onMessage(xpathInput())).resolves.toEqual({ output: 'hello', queryAll: false });
    });
    ...
    it('formats a single ordered node result', async () => {
      ...
      await expect(createListener().onMessage(xpathInput())).resolves.toEqual({ output: 'x', queryAll: false });
    });
    ...
    it('returns only the first node of a snapshot when queryAll is not set', async () => {
      ...
      await expect(createListener().onMessage(xpathInput())).resolves.toEqual({ output: 'a', queryAll: false });
    });
    ...
    it('returns an empty string for an unhandled result type', async () => {
      ...
      await expect(createListener().onMessage(xpathInput())).resolves.toEqual({ output: '', queryAll: false });
    });
  });
});
