...
export class FakeBrowserStorageArea {
  ...
  async getBytesInUse(keys?: string | string[] | null): Promise<number> {
    const keysInUse = keys == null ? Object.keys(this.data) : Array.isArray(keys) ? keys : [keys];

    return keysInUse.reduce(
      (total, key) =>
        Object.hasOwn(this.data, key) ? total + key.length + JSON.stringify(this.data[key]).length : total,
      0,
    );
  }
