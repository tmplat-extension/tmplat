...
- The content-side listener exists and is bound
  (`src/lib/content/any-content/any-content.config.ts:44`, `tab-content-message-listener.ts`).
...
- `capitalize`: legacy `utils.capitalize` title-cased **every** word; the new entry uses `es-toolkit`'s `capitalize`
...
### 2.6 Bare `{dateTime}`, `{lastModified}` and `{shorten}` no longer work (verified 2026-09-10)

All three worked without a section body in 1.x, and all three still declare a `Standard` category, so the guide
continues to advertise the bare form. They now render the renderer's **JavaScript source** instead.

- Legacy's engine (mustache.js 0.7.2, `8f3c5e3:src/vendor/mustache.js:294-297`) _invoked_ a nested function found
  by a name tag; `tmplat-mustache` 2.5.0 stringifies it (`decoratedValue`, `:574-597`).
- Legacy's `rendered` helper (`8f3c5e3:src/lib/background.coffee:651-655`) explicitly handled that zero-argument
  call via `if arguments.length ... else do callback`; the new factories in
  `src/lib/template/context/template-context.utils.ts` have no equivalent guard.
- The new mappers already contain the `content ? ... : <default>` fallbacks intended for bare use
  (`entry/date-time.ts`, `entry/last-modified.ts`, `entry/shorten.ts`) — they are simply unreachable.
- Predefined templates are unaffected (1.x shipped `{#shorten}{url}{/shorten}`), but user templates carried over
  from 1.x are.

Full analysis, and the verified two-part fix (a `tmplat-mustache` change plus a one-method change here), are in
[TEMPLATE-OPERATION-LEAK.md](TEMPLATE-OPERATION-LEAK.md).
