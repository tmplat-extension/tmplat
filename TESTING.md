...
1. `EuropaMarkdownService.convert` — currently covered only as far as its empty-input guard. Now unblocked by the
   `dom` project's jsdom environment, but the service itself lives in the `node` project.
2. `src/lib/ui/migrate/` — the last remaining 0% UI tree. Blocked on §1.1 of
   [MIGRATION-GAPS.md](MIGRATION-GAPS.md), since the page does not yet render anything.
