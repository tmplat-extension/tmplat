...
// Every locale the Chrome Web Store recognises, from
// https://developer.chrome.com/docs/extensions/reference/api/i18n#locales. Chrome silently ignores an unknown
// "_locales" directory, but the Chrome Web Store rejects the whole package as invalid, so anything not listed here
// must never reach "dist/temp".
...
// Crowdin names every translation after its full locale (`de_DE`, `fr_FR`, `sr_SP`), while the Chrome Web Store
// only accepts the handful of region-qualified codes listed above and expects the bare language everywhere else.
// Falling back to the language keeps those translations, since Chrome resolves `de_DE` to `de` anyway, and returns
// nothing for a language neither form supports (such as Afrikaans) so the caller can drop it.
  ...
  return {
    ...
    writeBundle() {
      ...
      for (const locale of readdirSync(localesSrc)) {
        if (!existsSync(getMessageFile(locale))) {
          continue;
        }
        ...
        if (!resolved) {
          this.warn(
            `Excluding the '${locale}' locale from the package as the Chrome Web Store does not support it, which ` +
              'would make the uploaded package invalid',
          );
          continue;
        }

        const conflict = resolvedLocales.get(resolved);
        if (conflict) {
          // Two locales of the same language (`de_AT` and `de_DE`, say) cannot both become `de`. Failing here is
          // deliberate: silently picking one would ship a translation nobody chose, and the fix belongs in the
          // Crowdin project rather than in whichever build happens to notice
          this.error(
            `The '${conflict}' and '${locale}' locales both map to '${resolved}'. Use 'languages_mapping' in ` +
              '"crowdin.yml" to give each a distinct locale the Chrome Web Store supports.',
          );
        }

        resolvedLocales.set(resolved, locale);
      }

      for (const [locale, srcLocale] of resolvedLocales) {
        const srcFile = getMessageFile(srcLocale);
        ...
        // A locale with nothing in it is what Crowdin produces for a language with no approved translations yet.
        // It carries no strings Chrome would not already take from `default_locale`, so shipping it only adds an
        // empty catalogue for the Chrome Web Store to object to
        if (locale !== defaultLocale && Object.keys(output).length === 0) {
          this.warn(`Excluding the '${locale}' locale from the package as it contains no translated i18n messages`);
          continue;
        }

        const destDir = joinPath(localesDest, locale);
        mkdirSync(destDir, { recursive: true });
        writeFileSync(joinPath(destDir, 'messages.json'), JSON.stringify(output, null, minify ? '' : 2));
      }
    },
  };
}
