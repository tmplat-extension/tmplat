...
- Add support for Microsoft Edge, published to the Microsoft Edge Add-ons store
- Add a migration page that transfers your templates and settings from version 1.x
- Add optional descriptions to templates so you can record what each one is for
- Add the ability to clone an existing template
- Add light, dark and system theme support
- Add optional notifications when tmplat is updated, with control over which releases you are told about
- Add support for the **spoo.me** and **da.gd** URL shortener services
- Add new **uuid** tag for generating a unique identifier
- Add new **camelCase**, **kebabCase**, **snakeCase**, **startCase** and **deburr** text functions
- Add new **encodeBase64** and **decodeBase64** functions, alongside explicit **escapeHtml** and **unescapeHtml** functions
- Add new **tabs** tag for iterating over the tabs open in the current window
- Add new **template** tag for accessing the details of the template being rendered
- Add new **pushUrl** and **popUrl** functions for building and modifying URL paths
- Add new **locale** tag for the browser's current language
- Add new **options** collection so a template can read any of your settings, replacing the individual option tags
- Add new **origin**, **browserMajorVersion**, **keyword** and **image** tags

### Improvements

- Rename the extension from _Template_ to _tmplat_, with a new website and store listings
- Rebuild the extension from scratch on Manifest V3 so that it starts faster and only runs when needed
- Rebuild the options page around a sortable template grid, with the search, bulk actions and import and export of 1.x carried over
- Copy without injecting into the page, so templates now also work on pages where the previous version silently failed
- Redesign the popup, which now reports the outcome of each template individually instead of showing a single progress bar
- Generate the guide from the template tags themselves and make it searchable, so it can no longer fall out of step with what the extension actually supports
- Sync your settings across devices, which 1.x never did; templates stay on the device they were created on and can be moved with export and import
- Replace the template import and export format with a more portable one; exports from 1.x can still be imported, but arrive as new user templates
- Change **capitalize** to only capitalise the first letter of the rendered content — use **startCase** for the previous behaviour
- Change **dateTime** and **lastModified** to use a new set of date format tokens, so existing format strings will need updating
- Change **encode** and **decode** to be aliases of the new **encodeUriComponent** and **decodeUriComponent** functions

### Deprecations

- Deprecate the older tag names in favour of clearer equivalents (e.g. **url** replaces **originalUrl**, and **trimStart** replaces **trimLeft**); every one still renders, so existing templates keep working
- Deprecate the individual option tags (e.g. **menu**, **shortcuts**, **yourlsUrl**) in favour of reading them from the **options** collection
- Deprecate the **bitly**, **bitlyAccount**, **googl**, **googlAccount**, **googlOAuth**, **java** and **popular** tags, which now always render `false` or nothing, rather than removing them and breaking templates that use them

### Removals

- Remove the **Bitly** URL shortener service, as its API is no longer reliable
- Remove the **goo.gl** URL shortener service, which Google has discontinued
- Remove the OAuth sign-in flow, which was only used by the Bitly and goo.gl services
- Remove all analytics and usage tracking; tmplat no longer phones home
- Remove the clipboard-read permission, which is no longer needed to copy
- Remove support for `file://` and `ftp://` pages, narrowing the permissions tmplat asks for to http and https pages only
- Remove per-template usage statistics, custom template images and the notification duration option
- Remove Java detection, which browsers no longer support
- Remove compatibility support for the IE Tab and Gecko Tab extensions
