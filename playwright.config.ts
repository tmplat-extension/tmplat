...
export default defineConfig({
  ...
  // Edge is Chromium, and the Edge Add-ons listing ships the *same* artifact as the Chrome Web Store rather than a
  // separate build, so it is covered as a second channel of one suite rather than a second build target. Both
  // projects load the identical `dist/temp`. Run one with `pnpm test:e2e --project=msedge`.
  projects: [
    {
      name: 'chromium',
